...
class IndexCreator:
    ...
    # ベクトルデータの保存先を作成
    def _create_vector_store(self, article_summary):
        metadata_filters = {"article_summary": article_summary}
    ...
    def insert_to_pinecone(self, article_summary, article):
        vector_store = self._create_vector_store(article_summary)
...
def create_index(article_summary, article, category_number_string):
    pinecone_index_name = os.getenv("PINECONE_INDEX_NAME")
    pinecone_environment = os.getenv("PINECONE_ENVIRONMENT")
    model_name = "text-embedding-ada-002"
    index_creator = IndexCreator(pinecone_index_name, pinecone_environment, model_name)
    pinecone_nodes = index_creator.insert_to_pinecone(article_summary, article)
    ...
    for i, node in enumerate(pinecone_nodes, start=1):
        ...
        category_id = f"{category_number_string}#{date_string}#{i:03}"
